"""端到端验收脚本:POST /api/chat,流式打印 SSE 事件与最终回复。

用法(先启动后端 python -m webapp):
    python webapp\\e2e_test.py "JTG 5120-2021 对桥梁检查周期是怎么规定的?"
    python webapp\\e2e_test.py "上一个问题的条文号是什么?" --session-id <上一条输出的 session id>
"""
...
def main() -> None:
    ...
    ap.add_argument("--session-id", default=None, help="缺省生成新会话;复用旧 id 即多轮")
    ap.add_argument("--port", type=int, default=8090)
    ap.add_argument("--timeout", type=float, default=600.0)
    args = ap.parse_args()

    session_id = args.session_id or f"e2e-{int(time.time())}"
    body = json.dumps({"session_id": session_id, "message": args.message})
    ...
    print(f"HTTP {resp.status} {resp.reason}  session={session_id}")
    if resp.status != 200:
    ...
    print(f"\n活动事件 {activities} 条;session={session_id}(同 id 再跑即多轮)")
